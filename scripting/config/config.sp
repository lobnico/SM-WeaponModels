		...
		if (keyValues.GotoFirstSubKey())
		{
			...
			do
			{
				...
				if (StringToIntEx(buffer, defIndex) != strlen(buffer))
				{
					...
					strcopy(g_Weapon_ClassName[weaponIndex], CLASS_NAME_MAX_LENGTH, buffer);
				}
				else if (defIndex < 0)
				{
				...
				g_Weapon_DefIndex[weaponIndex] = defIndex;

				#if defined DEBUG_WEAPONSWAP
				//CleanUpSwapWeapon(weaponIndex);
				#endif
				
				keyValues.GetString("ViewModel", g_Weapon_ViewModel[weaponIndex], PLATFORM_MAX_PATH + 1);
				keyValues.GetString("WorldModel", g_Weapon_WorldModel[weaponIndex], PLATFORM_MAX_PATH + 1);

				PrecacheWeaponInfo(weaponIndex);

				g_Weapon_TeamNum[weaponIndex] = KvGetNum(keyValues, "TeamNum");
				g_Weapon_BlockLAW[weaponIndex] = KvGetNum(keyValues, "BlockLAW") > 0;

				g_Weapon_SequenceCount[weaponIndex] = -1;

				g_Weapon_Status[weaponIndex] = STATUS_CONFIG;
